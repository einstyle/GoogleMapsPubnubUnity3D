...
public class PubnubExample : MonoBehaviour
{
    ...
    private RawImage mapCenterPointerImage;
    enum PubnubState
    {
        None,
        GrantSubscribe,
    ...
    string cipherKey = "";
    string secretKey = "";
    string publishKey = "YOUR_KEY";
    string subscribeKey = "YOUR_KEY";
    ...
    void Start ()
    {
        ...
        mapCenterPointerImage = googleMapComponent.gameObject.GetComponentInChildren<RawImage>();
        //---  end
    }
                ...
                if ((message != null)&&(message.Length >= 0)){
                    {
                        ...
                        foreach (object item in enumerable)
                        {
                            ...
                            UnityEngine.Debug.Log(string.Format("Message STRING: {0}", item.ToString()));
                            //IF CUSTOM OBJECT IS EXCEPTED, YOU CAN CAST THIS OBJECT TO YOUR CUSTOM CLASS TYPE
                        }
                    }
                }
    ...
    {
        UnityEngine.Debug.Log (string.Format ("DisplayReturnMessage REGULAR CALLBACK LOG: {0}", result));
        AddToPubnubResultContainer (string.Format ("REGULAR CALLBACK: {0}", result));
        SendDataToMap(result);
    }

    // Maps interaction:
    void SendDataToMap(string result)
    {
        
        UnityEngine.Debug.Log("Received result: " + result);
    ...
    void accessData(JSONObject obj)
    {
        ...
        switch (obj.type)
        {
            case JSONObject.Type.ARRAY:
                ...
                UnityEngine.Debug.Log("obj.list[0]: " + j);
                JSONObject lat = (JSONObject)j.list[0];
                JSONObject lon = (JSONObject)j.list[1];
                string lat_string = lat.ToString();
                string lon_string = lon.ToString();
                UnityEngine.Debug.Log("lat_string: " + lat_string);
                UnityEngine.Debug.Log("lon_string: " + lon_string);
                LatitudeText.text = lat_string;
                LongitudeText.text = lon_string;
                googleMapLocation.latitude = float.Parse(lat_string);
                googleMapLocation.longitude = float.Parse(lon_string);
                googleMapComponent.centerLocation.latitude = float.Parse(lat_string);
                googleMapComponent.centerLocation.longitude = float.Parse(lon_string);
    ...
    {
        UnityEngine.Debug.Log (string.Format ("DisplayReturnMessage obj REGULAR CALLBACK LOG: {0}", result));
        AddToPubnubResultContainer (string.Format ("REGULAR CALLBACK: {0}", result));
    }
    ...
    void DisplayReturnMessageObj (object result)
    {
        ...
        var stringList = myList.OfType<string>();
        string result2 = string.Join(",", stringList.ToArray());
        UnityEngine.Debug.Log (string.Format ("DisplayReturnMessageObj REGULAR CALLBACK LOG: {0}", result2));
        AddToPubnubResultContainer (string.Format ("DisplayReturnMessageObj REGULAR CALLBACK: {0}", result2));
    }
